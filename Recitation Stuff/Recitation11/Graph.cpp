...
void Graph::removeEdge(int key1, int key2)
{
  ...
  for(int b = 0; b < v2->adj.size(); b++)
  {
      if(v2->adj[b].v == v1)
      {
        v2->adj.erase(v2->adj.begin() + b);
        break;
      }
  }
...
bool Graph::isBridge(int key1, int key2)
{
  ...
  // TODO Step 1: Get initial_components. Complete and use the DFTraversal function.
    for(int i = 0; i < vertices.size(); i++)
    {
  ...
  // TODO Step 4: Add the edge back to the graph
  addEdge(key1, key2);

  // TODO Step 5: check if the no of connected components increases after removing the edge and return true
